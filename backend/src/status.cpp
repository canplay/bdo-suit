...
void Status::overview(
  ...
  auto stmt2 =
      fmt::format("SELECT COUNT (_characterNo) FROM "
                  ...
                  "TblBriefCharacterInformation] WHERE [_deletedDate] IS NULL");
  auto stmt3 = fmt::format("SELECT COUNT (_guildNo) FROM "
                           "[SA_BETA_GAMEDB_0002].[PaGamePrivate].[TblGuild]");
  auto stmt4 =
      fmt::format("SELECT TOP 10 * FROM "
                  "[SA_BETA_WORLDDB_0002].[PaGamePrivate].["
                  "TblCurrentInformation] ORDER BY [_updatedDate] DESC");
  auto stmt5 =
      fmt::format("SELECT COUNT (_mainKey) FROM "
                  "[SA_BETA_TRADEDB_0002].[PaGamePrivate].[TblWorldMarket]");
