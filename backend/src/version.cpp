...
void Version::count(
  ...
  auto stmt = fmt::format(
      "SELECT COUNT(id) FROM [SA_BETA_WORLDDB_0002].[PaWebPublic].[version]");
...
void Version::add(const HttpRequestPtr &req,
  ...
  async_run([=]() -> Task<> {
    ...
    try {
      ...
      if ((*json)["name"].asString() != "app") {
        char buf[10240];
        auto hdiff = std::make_shared<FILE *>(_popen(
            fmt::format(
                "diff -s-512m -p-2 -c-lzma -f \"{}\" \"{}\" \"{}/{}_{}.patch\"",
                (*json)["old"].asString(), (*json)["new"].asString(),
                (*json)["save"].asString(), (*json)["name"].asString(),
      ...
      auto timestamp =
          fmt::format("{:%Y-%m-%d %H:%M:%S}", fmt::localtime(time));

      auto stmt = fmt::format(
          "INSERT INTO [SA_BETA_WORLDDB_0002].[PaWebPublic].[version] ([id], "
          "[name], [version], [force], [link], [create_date], [create_user], "
          "[update_date], [update_user], [delete], [oldPath], [newPath], "
          "[savePath]) VALUES ('{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', "
          "'{}', 0, '{}', '{}', '{}');",
          uuidSimple(), (*json)["name"].asString(),
          (*json)["version"].asString(), (*json)["force"].asInt64(),
          (*json)["link"].asString(), timestamp,
...
void Version::update(
  ...
  auto stmt = fmt::format(
      "UPDATE [SA_BETA_WORLDDB_0002].[PaWebPublic].[version] SET [name] = "
      "'{}', [version] = '{}', [force] = {}, [link] = '{}', [update_date] = "
      "'{}', [update_user] = '{}', [delete] = {}, [oldPath] = '{}', [newPath] "
      "= '{}', [savePath] = '{}' WHERE [id] = '{}';",
      (*json)["name"].asString(), (*json)["version"].asString(),
      (*json)["force"].asInt64(), (*json)["link"].asString(), timestamp,
      (*json)["update_user"].asString(), (*json)["delete"].asInt64(),
      (*json)["oldPath"].asString(), (*json)["newPath"].asString(),
      (*json)["savePath"].asString(), (*json)["id"].asString());
...
void Version::info(
  ...
  std::string stmt = fmt::format(
      "SELECT TOP {} * FROM [SA_BETA_WORLDDB_0002].[PaWebPublic].[version] "
      "WHERE id NOT IN(SELECT TOP {} id FROM "
      "[SA_BETA_WORLDDB_0002].[PaWebPublic].[version]",
...
void Version::latest(
  ...
  try {
    ...
    std::string stmt = fmt::format(
        "SELECT TOP 1 * FROM [SA_BETA_WORLDDB_0002].[PaWebPublic].[version] "
